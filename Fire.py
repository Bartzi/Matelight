...
from rendering.display import MatelightEmulator, MatelightDisplay
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Tool that renders fire on the matelight")
    parser.add_argument('config', help="Path to config for matelight")
    parser.add_argument('-e', '--emulate', action='store_true', default=False, help="use emulator instead of real matelight")

    args = parser.parse_args()

    if args.emulate:
        display = MatelightEmulator(args.config)
    else:
        display = MatelightDisplay(args.config)
    ...
    try:
        ...
        pygame.init()
        spheres = (
            Fire(surface, pygame.Rect(0, 0, surface.get_width(), surface.get_height()), 1),
            )
